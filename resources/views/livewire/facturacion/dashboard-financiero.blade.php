<div>
    <div class="container-fluid py-4">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <select wire:model.live="rangoFechas" class="form-select">
        ...
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <button wire:click="$toggle('mostrarPagosPorUsuario')" class="btn btn-sm btn-primary">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="text-center">No hay pagos registrados en el período
                                            seleccionado</td>
                                    </tr>
                                @endforelse
                            </tbody>
                            <tfoot>
                                <tr>
                                    <th>Total</th>
                                    <th class="text-end">{{ collect($estadisticas['pagosPorUsuario'])->sum('count') }}
                                    </th>
                                    <th class="text-end">
                                        ${{ number_format(collect($estadisticas['pagosPorUsuario'])->sum('total'), 2) }}
                                    </th>
                                </tr>
                            </tfoot>
                        </table>
    ...
    @if ($usuarioSeleccionado)
        @php
            ...
            $nombreUsuario = $user
                ? $user->name ?? ($user->email ?? 'Usuario #' . $usuarioSeleccionado)
                : 'Desconocido';
        @endphp
                                ...
                                <div class="row">
                                    <div class="col-md-4 mb-3">
                                        <div class="border p-3 rounded bg-light">
                                            ...
                                            <p class="text-center mb-1 fw-semibold">Cantidad:
                                                {{ $resumenPagos['efectivo']['cantidad'] }}</p>
                                            <p class="text-center mb-0 fw-bold">
                                                ${{ number_format($resumenPagos['efectivo']['total'], 2) }}</p>
                                        </div>
                                    </div>
                                    <div class="col-md-4 mb-3">
                                        <div class="border p-3 rounded bg-light">
                                            ...
                                            <p class="text-center mb-1 fw-semibold">Cantidad:
                                                {{ $resumenPagos['transferencia']['cantidad'] }}</p>
                                            <p class="text-center mb-0 fw-bold">
                                                ${{ number_format($resumenPagos['transferencia']['total'], 2) }}</p>
                                        </div>
                                    </div>
                                    <div class="col-md-4 mb-3">
                                        <div class="border p-3 rounded bg-light">
                                            ...
                                            <p class="text-center mb-1 fw-semibold">Cantidad:
                                                {{ $resumenPagos['tarjeta']['cantidad'] }}</p>
                                            <p class="text-center mb-0 fw-bold">
                                                ${{ number_format($resumenPagos['tarjeta']['total'], 2) }}</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="mt-3 p-3 bg-primary text-white rounded">
                                    <h5 class="mb-0 text-end fw-bold">Total General:
                                        ${{ number_format($resumenPagos['total_general'], 2) }}</h5>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <tr>
                                            <td>
                                                <span class="badge bg-primary">
                                                    {{ \Carbon\Carbon::parse($pago->factura->fecha_emision)->locale('es')->translatedFormat('F Y') }}
                                                </span>
                                            </td>
                                            ...
                                            <td>
                                                ...
                                                @endphp
                                                <span
                                                    class="badge {{ $badgeClass }}">{{ ucfirst($pago->metodo_pago) }}</span>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="5" class="text-center py-4">No se encontraron pagos para
                                                este usuario en el período seleccionado</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
